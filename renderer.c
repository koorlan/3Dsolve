...
void* renderer ( void *arg )
{
	...
	while (app->running && !glfwWindowShouldClose (context->window))
	{
		...
		case AS_HELP:
			...
			glColor4f (1.f, 1.f, 1.f, 1.f);
			ftglRenderFont( textFont, "Flèche directionnelle haut : Ré-initialise le serpent", FTGL_RENDER_ALL);
			glLoadIdentity();
			glTranslatef (myX, 40,0.f);
			glColor4f (1.f, 1.f, 1.f, 1.f);
			ftglRenderFont( textFont, "Flèche directionnelle bas : échanger entre mode joueur et mode résolution", FTGL_RENDER_ALL);
			glLoadIdentity();
			...
			glTranslatef (myX, -160,0.f);
			glColor4f (1.f, 1.f, 1.f, 1.f);
			ftglRenderFont( textFont, "Echap : Quitter l'application", FTGL_RENDER_ALL);
